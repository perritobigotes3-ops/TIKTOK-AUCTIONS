...
function registerDonation(username, coins, avatar, idKey) {
  if (!state.running) {
    console.log(`⏸ Donación ignorada (subasta no activa): ${username} → ${coins}`);
    return;
  }

  if (idKey && processedDonations.has(idKey)) {
    console.log(`⚠️ Donación duplicada ignorada: ${idKey}`);
    return;
  }
  if (idKey) processedDonations.add(idKey);
...
async function simulateDonation(username, coins) {
  ...
  registerDonation(username, coins, avatar || null, `sim-${Date.now()}-${Math.random()}`);
}
  ...
  try {
    ...
    tiktokConn.on('gift', async (data) => {
      try {
        ...
        if (data.repeatEnd) {
          console.log(`⏩ Evento repeatEnd ignorado para ${username}`);
          return;
        }

        console.log(`💎 Donación procesada: ${username} → ${coins}`);
        const avatar = data.profilePictureUrl && data.profilePictureUrl.trim() !== ''
          ? data.profilePictureUrl
          : null;

        registerDonation(username, coins, avatar, giftId);
      } catch (err) {
